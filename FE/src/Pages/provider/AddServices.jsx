...
const AddServices = ({ isModal = false, onClose, onCreated } = {}) => {
  ...
  const validateFields = () => {
    ...
    if (!itineraryFile) nextErrors.itineraryFile = "Vui lòng chọn file Excel lịch trình";
  ...
  return (
    <div className={isModal ? "" : "min-h-screen bg-[#f8fafc]  md:px-6 md:py-10"}>
      <div className={isModal ? "w-full" : "max-w-4xl"}>
        {!isModal ? (
          <div className="mb-6  gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex justify-center">
          <div className={`relative w-full border border-orange-100 bg-white p-6 shadow-sm md:p-8 ${
            ...
            {isModal ? (
             ...
             <div>
            <h1 className="text-3xl mb-5  font-bold text-gray-900">Thêm dịch vụ mới</h1>
          </div>
            {message && (
              <div
  ...
  <label className={labelClass}>
    <RequiredLabel>Lịch trình Excel</RequiredLabel>
  </label>
  ...
  <div className="relative">
    <input
      ...
      className={`block w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 pr-12 text-sm text-gray-700 outline-none transition file:mr-4 file:rounded-lg file:border-0 file:bg-orange-50 file:px-3 file:py-2 file:text-sm file:font-semibold file:text-orange-600 focus:border-orange-400 focus:bg-white focus:ring-2 focus:ring-orange-100 ${
        errors.itineraryFile
    ...
    <button
      ...
      className="absolute right-3 top-1/2 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-lg text-gray-400 transition hover:bg-red-50 hover:text-red-500"
