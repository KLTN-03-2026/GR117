...
const ServiceManagement = () => {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        const result = await axios.get("http://localhost:5000/api/admin/services", {
          headers: {
  ...
  const filtered = services
    .filter((item) => {
      ...
      const provider = String(item.nameProvider || "").toLowerCase();
    ...
    try {
      ...
      if (action === "approve") {
        const result = await axios.patch(
          `http://localhost:5000/api/admin/services/${id}/approve`,
          {},
      ...
      if (action === "reject") {
        ...
        const result = await axios.patch(
          `http://localhost:5000/api/admin/services/${id}/reject`,
          {},
          { headers: { Authorization: `Bearer ${accessToken}` } },
        );
